...
class PrivacyAgent:
  ...
  def preliminary_censor(self,text):
    try:
      ...
      return cleaned_text, cleaned_text != text
  ...
  def complete_censor(self,text,bad_words=[],replacements=[]):
    text, A = self.preliminary_censor(text)
    text, B = self.in_depth_censor(text)
    text, C = self.custom_regex_censor(text,bad_words,replacements)
    return text, A or B or C
