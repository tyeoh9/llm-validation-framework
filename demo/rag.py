...
class RAG:
    ...
    def __init__(
        ...
        self.index_dir = Path(index_dir) if index_dir else Path.cwd() / "faiss_index"
    ...
    def extract_docs(self, query: str, threshold: float, k: int = 5) -> str:
        """
        Retrieve top-k chunks with scores, keep only chunks with score <= threshold,
        clean their text, and concatenate them into one string.

        Note:
        For FAISS similarity_search_with_score, lower score usually means more similar.
        """
        if self._vectorstore is None:
            self.build_or_load_vectorstore()

        scored_results = self._vectorstore.similarity_search_with_score(query, k=k)
        ...
        for doc, score in scored_results:
            if score <= threshold:
                ...
                if cleaned_text:
                    filtered_chunks.append(cleaned_text)
    ...
    def save_results_to_file(
        ...
        output_path = Path(filename) if filename else Path.cwd() / "retrieval_output.txt"
